...
void ReadProblem()
{
    ...
    // 调整参数，设置默认值
    if (Seed == 0)
        Seed = (unsigned)(time(0) * (size_t)(&Seed)); // 初始化随机种子
    if (Precision == 0)
        Precision = 100; // 设置精度
    if (InitialStepSize == 0)
        InitialStepSize = 1; // 初始化步长
    if (MaxSwaps < 0)
        MaxSwaps = Dimension; // 设置最大交换次数
    if (KickType > Dimension / 2)
        KickType = Dimension / 2; // 设置踢击类型
    if (Runs == 0)
        Runs = 10; // 设置运行次数
    if (MaxCandidates > Dimension - 1)
        MaxCandidates = Dimension - 1; // 设置最大候选数
    if (ExtraCandidates > Dimension - 1)
        ExtraCandidates = Dimension - 1; // 设置额外候选数
    if (SubproblemSize >= Dimension)
        SubproblemSize = Dimension; // 设置子问题大小
    else if (SubproblemSize == 0)
    {
        if (AscentCandidates > Dimension - 1)
            AscentCandidates = Dimension - 1; // 设置上升候选数
        if (InitialPeriod < 0)
        {
            InitialPeriod = Dimension / 2; // 设置初始周期
            if (InitialPeriod < 100)
                InitialPeriod = 100; // 确保初始周期最小值
        }
        if (Excess < 0)
            Excess = 1.0 / Dimension; // 设置过量
        if (MaxTrials == -1)
            MaxTrials = Dimension; // 设置最大试验次数
        HeapMake(Dimension);       // 创建堆
    }

    // 处理问题参数和计算成本矩阵
    if (POPMUSIC_MaxNeighbors > Dimension - 1)
        POPMUSIC_MaxNeighbors = Dimension - 1; // 设置最大邻居数
    if (POPMUSIC_SampleSize > Dimension)
        POPMUSIC_SampleSize = Dimension; // 设置样本大小
        ...
        Distance != Distance_ATSP && Distance != Distance_SPECIAL)
    {
        ...
        CostMatrix = (int *)calloc((size_t)Dimension * (Dimension - 1) / 2,
                                   sizeof(int)); // 分配成本矩阵
        Ni = FirstNode->Suc;                     // 从第一个节点开始
        do
        {
            Ni->C =
                &CostMatrix[(size_t)(Ni->Id - 1) * (Ni->Id - 2) / 2] - 1; // 计算成本矩阵索引
            if (ProblemType != HPP || Ni->Id < Dimension)
                for (Nj = FirstNode; Nj != Ni; Nj = Nj->Suc)
                    Ni->C[Nj->Id] = Fixed(Ni, Nj) ? 0 : Distance(Ni, Nj); // 计算边的距离
            else
                for (Nj = FirstNode; Nj != Ni; Nj = Nj->Suc)
                    Ni->C[Nj->Id] = 0; // 特殊情况处理
        } while ((Ni = Ni->Suc) != FirstNode); // 遍历所有节点
        WeightType = EXPLICIT; // 设置权重类型
        c = 0;
    }
        ...
        for (i = 2; i <= n; i++)
        {
            ...
            for (j = 1; j < i; j++)
                if (N->C[j] * Precision / Precision != N->C[j])
                    eprintf("PRECISION (= %d) is too large", Precision); // 精度过大
        }
    }
    ...
    if (PatchingC >= 1)
    {
        BestMove = BestSubsequentMove = BestKOptMove; // 如果修补

        // 如果不进行后续修补且后续移动类型小于等于5
    ...
    // 如果问题类型是HCP或HPP，设置最大候选数为0
    if (ProblemType == HCP || ProblemType == HPP)
        MaxCandidates = 0;

    // 追踪级别大于等于1时输出完成信息并打印参数
    if (TraceLevel >= 1)
    {
        printff("done\n");
        PrintParameters(); // 打印当前参数
    }
    else
        printff("PROBLEM_FILE = %s\n",
                ProblemFileName ? ProblemFileName : ""); // 打印问题文件名
    ...
    if (MergeTourFiles >= 1)
    {
        free(MergeTourFile);                                              // 释放旧的合并路径文件指针
        MergeTourFile = (FILE **)malloc(MergeTourFiles * sizeof(FILE *)); // 为新的合并路径文件分配内存
        for (i = 0; i < MergeTourFiles; i++)
            ReadTour(MergeTourFileName[i], &MergeTourFile[i]); // 读取每个合并路径文件
    }

    // 释放最后一行缓冲区
    free(LastLine);
    LastLine = 0; // 将其设置为NULL以避免悬空指针
}
...
static void CheckSpecificationPart()
{
    ...
    if (WeightType != EXPLICIT && (WeightType != SPECIAL || CoordType != NO_COORDS) && WeightType != -1 && WeightFormat != -1 && WeightFormat != FUNCTION)
        eprintf("Conflicting EDGE_WEIGHT_TYPE and EDGE_WEIGHT_FORMAT");
    ...
    if (CandidateSetType == NN && !TwoDWeightType() && !ThreeDWeightType() && MaxCandidates > 0)
        eprintf("Illegal EDGE_WEIGHT_TYPE for CANDIDATE_SET_TYPE = "
                "NEAREST-NEIGHBOR");
    ...
    if (ExtraCandidateSetType == NN && !TwoDWeightType() && !ThreeDWeightType() && ExtraCandidates > 0)
